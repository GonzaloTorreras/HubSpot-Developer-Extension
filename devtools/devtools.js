	...
	function (panel) {
		...
		panel.onShown.addListener(function (window) {
			...
			chrome.devtools.inspectedWindow.eval(
				"document.querySelector('.hs-tools-menu a[href*=\"/___context___/\"]').getAttribute('href')",
				function (elementResult) {
					...
					fetch(href)
						...
						.then(function (text) {
							window.document.querySelector('h1').innerText =
								'Formatting dev Info...';
                    ...
                    `;
                            ...
                            */
							...
							async function generateEvents() {
								// add event listeners
								const wrappers =
									window.document.querySelectorAll('[data-wrapper="object"]');
								...
								uls.forEach((ul) => {
									ul.addEventListener('mouseover', async function () {
										...
										if (
											ul.previousSibling &&
											ul.previousSibling.classList.contains('key')
										) {
							...
							async function generateSnippet(key) {
								...
								window.document.querySelectorAll('.selected').forEach((li) => {
									if (li.parentElement.classList.contains('list')) {
										const index = Array.from(li.parentNode.children).indexOf(
											li
										);
